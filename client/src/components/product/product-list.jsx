...
const ProductList = () => {
    const {ListProduct,BrandList,CategoryList,BrandListRequest,CategoryListRequest,ListByFilterRequest}=ProductStore();
                ...
                <div className="col-md-3 p-2">
                    <div className="card vh-100 p-3 shadow-sm">
                        ...
                        <select className="form-control form-select"
                                value={Filter.brandID} onChange={async (e)=>{await InputOnChange('brandID',e.target.value)}}
                        >
                        ...
                        <input  value={Filter.priceMax} onChange={async (e)=>{await InputOnChange('priceMax',e.target.value)}}
                            min={0} max={1000000} step={1000} type="range" className="form-range"/> <label
                        className="form-label mt-3">Minimum Price ${Filter.priceMin}</label>
                        <input value={Filter.priceMin} onChange={async (e)=>{await InputOnChange('priceMin',e.target.value)}}
                            min={0} max={1000000} step={1000} type="range" className="form-range"/></div>
                </div>
